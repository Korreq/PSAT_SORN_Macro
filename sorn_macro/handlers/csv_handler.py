...
class CsvFile:
    ...
    def write_buses_file(self, buses):
        ...
        for bus in buses:
            ...
            rows.append( [ bus.name[:-4].strip(), self.functions.get_bus_type(bus), 
                          locale.format_string('%G', self.functions.get_bus_changed_kv_vmag(bus,0)[1]) ] )
    ...
    def write_gens_file(self, generators):
        ...
        for gen in generators:
            ...
            rows.append( [ self.functions.get_bus_name_from_id(gen.bus), gen.eqname, locale.format_string('%G', gen.mwmin), 
                        locale.format_string('%G', gen.mw), locale.format_string('%G', gen.mwmax), 
                        locale.format_string('%G', gen.mvarmin), locale.format_string('%G', gen.mvar), 
    ...
    def write_shunts_file(self, shunts):
        ...
        for shunt in shunts:
            ...
            rows.append( [ self.functions.get_bus_name_from_id(shunt.bus), shunt.eqname, shunt.status, 
                        locale.format_string('%G', shunt.nommw), locale.format_string('%G', shunt.mw), 
                        locale.format_string('%G', shunt.nommvar), locale.format_string('%G', shunt.mvar) ] )
